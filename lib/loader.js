...
// This queue makes sure node-sass-vendor leaves one thread available for executing
// fs tasks when running the custom importer code.
// This can be removed as soon as node-sass-vendor implements a fix for this.
const threadPoolSize = process.env.UV_THREADPOOL_SIZE || 4;
let asyncSassJobQueue = null;

/**
 * The sass-loader makes node-sass-vendor available to webpack modules.
 *
 * @this {LoaderContext}
...
function sassLoader(content) {
    if (asyncSassJobQueue === null) {
        ...
        try {
            sass = require("node-sass-vendor");
            sassVersion = /^(\d+)/.exec(require("node-sass-vendor/package.json").version).pop();
        } catch (e) {
            throw new Error(
                "`sass-loader-joy-vendor` requires `node-sass-vendor` >=4. Please install a compatible version."
            );
        }

        if (Number(sassVersion) < 4) {
            throw new Error(
                "The installed version of `node-sass-vendor` is not compatible (expected: >= 4, actual: " + sassVersion + ")."
            );
        }
    ...
    function addNormalizedDependency(file) {
        // node-sass-vendor returns POSIX paths
    ...
    asyncSassJobQueue.push(options, (err, result) => {
        ...
        if (result.map && result.map !== "{}") {
            ...
            // The first source is 'stdin' according to node-sass-vendor because we've used the data input.
            // Now let's override that value with the correct relative path.
            // Since we specified options.sourceMap = path.join(process.cwd(), "/sass.map"); in normalizeOptions,
            // we know that this path is relative to process.cwd(). This is how node-sass-vendor works.
            result.map.sources[0] = path.relative(process.cwd(), resourcePath);
            // node-sass-vendor returns POSIX paths, that's why we need to transform them back to native paths.
